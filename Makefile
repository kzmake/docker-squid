...
VERSION      ?= $(VERSION_CORE)$(if $(PRE_RELEASE),-$(PRE_RELEASE),)$(if $(BUILD),+$(BUILD),0.0.0)
...
restart: ## Restart container
	docker ps -a | grep '$(CNTNAME)' -q && docker restart $(CNTNAME) || echo "Service not running."
...
rdebug: ## Exec shell as root in container
	docker exec -u root -it $(CNTNAME) $(SHCOMMAND)
